...
export default function Teacher_create_task() {
  ...
  useEffect(() => {
    if (editingTask) {
      ...
      setDueDate(editingTask.due_date || "No Due Date");
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      const taskData = {
        ...
        ...(dueDate !== "No Due Date" && { due_date: dueDate }),
        attachments,
        links,
      ...
      if (editingTask) {
        ...
        const { error } = await updateAssignmentTask(
          editingTask.id,
          taskData
        );
      ...
      <div className="flex-1 flex gap-6 overflow-hidden min-h-0">
            ...
            <div className="flex gap-6 justify-center">
              <label className="flex flex-col items-center gap-2 transition-transform cursor-pointer hover:scale-110">
                <div className="w-14 h-14 rounded-full border border-slate-200 flex items-center justify-center shadow-sm">
                  <img src="/upload.png" alt="Upload" className="w-7 h-7 object-contain" />
                </div>
                <span className="text-xs font-medium text-[#475569]">Upload</span>
                <input
                  type="file"
                  multiple
                  className="hidden"
              ...
              >
                <div className="w-14 h-14 rounded-full border border-slate-200 flex items-center justify-center shadow-sm">
                  <img src="/link.png" alt="Link" className="w-7 h-7 object-contain" />
                </div>
        ...
        <div className="w-80 bg-white rounded-2xl p-6 flex flex-col gap-6 shadow-sm border border-slate-200">
          ...
          <div className="flex flex-col gap-2">
            <h3 className="font-semibold text-[#1e293b] text-sm">Rubric (PDF)</h3>
            {rubricUrl ? (
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              {showDatePicker && (
                <div className="absolute top-full left-0 mt-2 z-50 bg-white border border-[#cbd5e1] rounded-lg shadow-lg p-2">
                  <input
                    type="date"
                    onChange={(e) => {
          ...
          >
            {isSubmitting ? (editingTask ? "Updating..." : "Creating...") : (editingTask ? "Update" : "Assign")}
          </button>
        </div>
      </div>
