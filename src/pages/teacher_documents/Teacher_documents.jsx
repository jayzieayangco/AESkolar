...
export default function Teacher_Documents() {
  ...
  // Controls individual file contextual modal popups natively
  const [showMenu, setShowMenu] = useState(false);
  const dropdownRef = useRef(null);

  // Manage your documents state here. 
  // Changing this to an array with items will automatically switch back to the grid view.
  ...
  // Triggers redirection to the teacher essay editor layout
  const handleCreateDocument = () => {
    navigate("/teacher_essay_editor");
  };

  // Safely close popup modal window if user clicks outside of bounds
  useEffect(() => {
      ...
      <div className="flex flex-1 w-full gap-8 overflow-hidden">
        ...
        <div className="flex-1 h-full flex flex-col gap-8 overflow-y-auto box-border pr-2 pb-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <button className="bg-white text-slate-800 font-medium py-3 px-8 rounded-xl shadow-[0_4px_6px_rgba(0,0,0,0.04)] cursor-pointer text-base transition-all duration-200 hover:shadow-md hover:scale-[1.01] active:scale-[0.99]">
              Upload File
            </button>
          </div>
